...
{
    /// <summary>
    /// Provides methods to convert double into string in binary format by IEEE 754 standard.
    /// </summary>
    public static class DoubleConverter
    {
        #region Privite consts
        ...
        #region Public methods of double convertor

        /// <summary>
        /// This method converts double into string in binary format.
        /// </summary>
        ...
        public static string ConvertToIEEE754(this double number)
        {
            ...
            if (IsNegative(number) || double.IsNaN(number))
            {
                sign = "1";
            ...
            double fractPart = GetMantissa(number, exponent);
        ...
        #region Private methods of double convertor

        private static bool IsNegative(double number)
        {
            if (number < 0 || (number == 0.0 && double.IsNegativeInfinity(1.0 / number)))
            {
                return true;
            }
        ...
        private static int GetExponent(double number)
        {
            ...
            double fractPart = (number / Math.Pow(2, power)) - 1;

            while (fractPart < 0 || fractPart >= 1)
            {
                power = fractPart < 1 ? --power : ++power;
                fractPart = (number / Math.Pow(2, power)) - 1;
            }
